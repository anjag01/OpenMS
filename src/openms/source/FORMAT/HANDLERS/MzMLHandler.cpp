        ...
        try
        {
            ...
            if (compress)
            {
                ...
                if (pigz_available)
                {
                    ...
                    int threads_to_use = (num_threads_ > 0) ? num_threads_ : omp_get_max_threads();
                    OPENMS_LOG_INFO << "Setting pigz to use " << threads_to_use << " threads" << std::endl;
                    ...
                    pigz_process = std::make_unique<bp::child>(
                        "pigz -c -p " + std::to_string(threads_to_use),
                        bp::std_in < *pigz_pipe,
